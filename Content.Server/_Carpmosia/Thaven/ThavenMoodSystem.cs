...
namespace Content.Server.Thaven;
...
public sealed partial class ThavenMoodsSystem : SharedThavenMoodSystem
{
    ...
    private ProtoId<DatasetPrototype> PositiveDataset = "ThavenMoodsPositive";
    private ProtoId<DatasetPrototype> NegativeDataset = "ThavenMoodsNegative";
    private ProtoId<DatasetPrototype> WildcardDataset = "ThavenMoodsWildcard";

    private EntProtoId ActionViewMoods = "ActionViewMoods";

    private ProtoId<WeightedRandomPrototype> RandomThavenMoodDataset = "ThavenMoodsRandom";
    ...
    {
        _sharedMoods.Clear();
        for (int i = 0; i < _config.GetCVar(CCVars.ThavenSharedMoodCount); i++)
    ...
    {
        // "Yes, and" moods
        if (TryPick(PositiveDataset, out var mood, GetActiveMoods(ent)))
            TryAddMood(ent, mood, true, false);

        // "No, and" moods
        if (TryPick(NegativeDataset, out mood, GetActiveMoods(ent)))
            TryAddMood(ent, mood, true, false);
