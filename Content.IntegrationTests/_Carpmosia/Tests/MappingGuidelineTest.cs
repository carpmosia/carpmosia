...
public sealed partial class MappingGuidelineTest : GameTest
{
    ...
    public async Task MappedEntitiesUnderWallsTest(ResPath map)
    {
        ...
        var wallmountProtos = Pair.GetPrototypesWithComponent<AtmosPipeColorComponent>().Select(x => x.Item1.ID).ToHashSet();
        ...
        Assert.Multiple(() =>
        {
            foreach (var proto in entities)
            {
                ...
                foreach (var ent in (YamlSequenceNode)proto["entities"])
                {
                    ...
                    Assert.That(wallPos,
                        Does.Not.Contain(trans),
                        $"\nMap {map} contains non-wallmount entitiy ({protoId}) mapped on wall ({trans})");
                }
            }
        });
    }
