reagent-name-angostura = Angostura
reagent-desc-angostura = Concentrated bitters based on herbs and spices. While being used for flavoring cocktails, bitters are not potable to drink on their own.

reagent-name-sweet-revenge = Sweet Revenge
reagent-desc-sweet-revenge = Strong, sweet and best served cold. 

reagent-name-old-fashioned = Old Fashioned
reagent-desc-old-fashioned = There are cocktails and there is Old Fashioned.

reagent-name-llb = LLB
reagent-desc-llb = Lemon, Lime and Bitters, almost non-alcoholic.

reagent-name-rob-roy = Rob Roy
reagent-desc-rob-roy = Scotch cousin of Manhattan with a dash of bitters.

reagent-name-gunner = Gunner
reagent-desc-gunner = The only real Hong Kong cocktail.

reagent-name-carribean-lovers = Carribean Lovers
reagent-desc-carribean-lovers = The drink to unite two people, coming with a couple's discount.

reagent-name-the-last-word = The Last word
reagent-desc-the-last-word = An elegant conclusion of the speech, perfect for making a public statement. 

reagent-name-coffee-flip = Coffee Flip
reagent-desc-coffee-flip = Perfect for breakfast to start a new day, but also works as a dessert cocktail.